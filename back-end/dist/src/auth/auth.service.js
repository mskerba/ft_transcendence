...
let AuthService = class AuthService {
    ...
    async generateTwoFactorAuthSecret(user) {
        ...
        const otpAuth = otplib_1.authenticator.keyuri(user.email, 'PongGreen', secret);
    ...
    async enableTwoFactorAuth(user, token) {
        ...
        if (user.twoFA_SecretKey) {
            const verified = otplib_1.authenticator.check(token, user.twoFA_SecretKey);
            if (verified) {
                await this.userService.update(user.userId, {
                    twoFA_Enabled: true,
                    twoFA_Verified: true,
    ...
    async verifyTwoFactorAuth(user, token) {
        ...
        const verified = otplib_1.authenticator.check(token, secretKey);
        if (verified) {
            await this.userService.update(user.userId, {
                twoFA_Verified: true,
            });
